...
type Product = {
  ...
  badge_color: string; active: boolean; images: string[]
...
export default function ServicesPage() {
  ...
  useEffect(() => {
    fetch('/api/inventory')
      ...
      .then(d => {
        ...
        active.forEach((p: Product) => {
          if (p.product_variants?.[0]) defaults[p.id] = p.product_variants[0].id
        })
  ...
  const inp: React.CSSProperties = {
    fontFamily: 'DM Sans, sans-serif', fontSize: 14, color: 'var(--bottle, #0f1f17)',
  }
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => setCategory(c.id)}
              style={{ padding: '8px 18px', fontSize: 13, fontWeight: 500, borderRadius: 40, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', border: category === c.id ? 'none' : '0.5px solid rgba(26,58,42,.2)', background: category === c.id ? '#1a3a2a' : 'transparent', color: category === c.id ? '#fff' : '#374151', transition: 'all .15s' }}>
              {c.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 40px 80px' }}>
        {loading && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
            {[1,2,3,4,5,6].map(i => (
              <div key={i} style={{ background: '#fff', borderRadius: 18, height: 280, border: '0.5px solid rgba(26,58,42,.08)', opacity: 0.5 }} />
            ))}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
          {filtered.map(product => {
            ...
            return (
              <div key={product.id} style={{ background: '#fff', borderRadius: 18, border: '0.5px solid rgba(26,58,42,.08)', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 2px 12px rgba(0,0,0,.04)', transition: 'box-shadow .2s' }}>

                {/* Image / Icon area */}
                <div style={{ position: 'relative', height: 140, background: 'linear-gradient(135deg, #f0faf4 0%, #e8f5ec 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {product.images?.[0] ? (
                  ...
                  {product.badge && (
                    <span style={{ position: 'absolute', top: 12, right: 12, fontSize: 11, fontWeight: 500, padding: '4px 10px', borderRadius: 20, background: product.badge_color, color: '#fff' }}>
                      {product.badge}
                    </span>
                  )}
                </div>
                ...
                <div style={{ padding: '20px 20px 16px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontSize: 16, fontWeight: 500, color: '#0f1f17', marginBottom: 6 }}>{product.name}</div>
                  <div style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.6, marginBottom: 14, flex: 1 }}>{product.description}</div>
                  ...
                  {isExpanded && (
                    <ul style={{ margin: '0 0 12px', padding: 0, listStyle: 'none' }}>
                      {product.includes?.map((item, i) => (
                        <li key={i} style={{ fontSize: 12, color: '#374151', padding: '4px 0', borderBottom: '0.5px solid rgba(26,58,42,.06)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {/* Price + CTA */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', paddingTop: 14, borderTop: '0.5px solid rgba(26,58,42,.06)' }}>
                    <div>
                      <span style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 22, fontWeight: 600, color: '#0f1f17' }}>
                        {selectedVariant?.price === 0 ? '0.4% fee' : `£${selectedVariant?.price ?? minPrice}`}
                      </span>
                      {product.product_variants?.length > 1 && !selectedVariantId && (
                        <span style={{ fontSize: 11, color: '#9ca3af', marginLeft: 4 }}>from</span>
                      )}
                    </div>
                    <Link href="/auth/signup"
                      style={{ fontSize: 13, fontWeight: 500, color: '#fff', background: '#1a3a2a', padding: '9px 18px', borderRadius: 40, textDecoration: 'none' }}>
                      Order now
                    </Link>
                  </div>
                </div>
